...
export default function App() {
  ...
  /* одноразовые эффекты (сердца/метла/пузыри) гаснут сами */
  useEffect(() => {
    if (!s.fx) return;
  ...
  /* самообучение нейросети во сне (нужен интернет) */
  const petSleeping = !!s.pet?.sleeping && !s.pet.transcended;
  useEffect(() => {
    if (!petSleeping) return;
    void engine.sleepLearn();
    const iv = setInterval(() => void engine.sleepLearn(), 20000);
  ...
  return (
    <div className="min-h-dvh lg:h-dvh lg:overflow-hidden" style={{ background: 'radial-gradient(ellipse at 20% 0%, #1c2a52 0%, #10172b 55%)' }}>
      ...
      {showLearning && <Learning petName={pet.name} onClose={() => setShowLearning(false)} />}
      ...
      <div className="lg:h-dvh lg:grid lg:grid-cols-[minmax(0,1fr)_440px] lg:gap-4 lg:p-4 lg:max-w-[1280px] lg:mx-auto">
        ...
        <div className="relative h-[46dvh] min-h-[330px] sm:h-[50dvh] sm:min-h-[380px] lg:h-auto lg:min-h-0 overflow-hidden border-b lg:border border-sky/10 lg:rounded-[28px]">
          <RoomScene themeId={s.roomTheme} furniture={s.furniture} phase={phase} weather={weather}
            ...
            {/* реплика — закреплена сверху сцены */}
            {s.bubble && (
            ...
            {pet.transcended ? (
              ...
              <div className="absolute inset-x-0 bottom-[84px] sm:bottom-[92px] lg:bottom-[150px] flex justify-center z-10">
                <div key={s.fx?.kind === 'pet' ? s.fx.at : 'pet'} className={s.fx?.kind === 'pet' ? 'anim-squish' : ''}>
                  <PetSprite pet={pet} size="min(50vw, 240px)" onStroke={() => engine.petStroke()} />
                </div>
              </div>
            )}

            {/* одноразовые эффекты ухода */}
        ...
        <div className="lg:h-dvh lg:overflow-y-auto no-scrollbar lg:pr-1">
          <div className="p-3.5 sm:p-4 pb-28 lg:pb-6 space-y-3.5 max-w-xl lg:max-w-none mx-auto">
            <HUD pet={pet} coins={s.coins} weather={weather}
              soundOn={s.settings.sound}
              onToggleSound={() => engine.setSound(!s.settings.sound)}
              onOpenSettings={() => setShowSettings(true)} />

            {tab === 'scene' && (
              <div className="card p-4 text-center anim-fade-up">
                <p className="text-[13px] font-bold text-cream/65 leading-relaxed">
                  {pet.name} сейчас в комнате. Погладьте его, выкупайте, уберитесь — или загляните во вкладки: там еда, игры, болталка и дневник.
                </p>
...
function FxLayer({ fx }: { fx: { kind: 'pet' | 'clean' | 'bath'; at: number } }) {
  ...
  if (fx.kind === 'pet') {
    const hearts = [
      { l: '38%', b: '52%', d: 0, r: -12 }, { l: '58%', b: '58%', d: 0.08, r: 14 },
      { l: '46%', b: '64%', d: 0.16, r: 0 }, { l: '34%', b: '44%', d: 0.24, r: -18 },
      { l: '62%', b: '48%', d: 0.3, r: 18 }, { l: '50%', b: '70%', d: 0.38, r: 6 },
    ];
    return (
      <div key={key} className="absolute inset-0 pointer-events-none z-30">
        {hearts.map((h, i) => (
          <span key={i} className="absolute fx-heart text-rose" style={{ left: h.l, bottom: h.b, animationDelay: `${h.d}s`, ['--hr' as string]: `${h.r}deg` }}>
  ...
  if (fx.kind === 'clean') {
    return (
      <div key={key} className="absolute inset-0 pointer-events-none z-30 overflow-hidden">
        <div className="absolute bottom-[12%] left-0 fx-broom" style={{ width: '64px' }}>
          <Icon name="broom" className="w-14 h-14 text-butter drop-shadow-lg" />
        </div>
        {[18, 34, 50, 66, 82].map((l, i) => (
          <span key={l} className="absolute fx-dust" style={{ left: `${l}%`, bottom: '12%', animationDelay: `${i * 0.18}s` }}>
  ...
  /* bath */
  const bubbles = [
    { l: '40%', b: '46%', s: 14, d: 0 }, { l: '55%', b: '50%', s: 20, d: 0.1 },
    { l: '47%', b: '56%', s: 10, d: 0.2 }, { l: '60%', b: '44%', s: 16, d: 0.28 },
    { l: '36%', b: '54%', s: 12, d: 0.36 }, { l: '52%', b: '62%', s: 18, d: 0.44 },
    { l: '44%', b: '42%', s: 9, d: 0.5 }, { l: '58%', b: '58%', s: 12, d: 0.56 },
  ];
  return (
    <div key={key} className="absolute inset-0 pointer-events-none z-30">
      {bubbles.map((b, i) => (
        <span key={i} className="absolute fx-bubble" style={{ left: b.l, bottom: b.b, animationDelay: `${b.d}s` }}>
