...
const WEATHER_ICON: Record<string, string> = { clear: 'sun', clouds: 'cloud', rain: 'rain', snow: 'snowflake', wind: 'wind' };
    ...
    <div className="space-y-3">
      ...
      <div className="flex items-center gap-1.5 flex-wrap -mt-1.5">
        ...
        {pet.evolutionTraits.length > 0 && (
          <span className="chip !text-[10.5px]" style={{ color: '#ffd98e' }}>
            <Icon name="spark" className="w-3 h-3" />{pet.evolutionTraits.length} черт{pet.evolutionTraits.length >= 2 && pet.evolutionTraits.length < 5 ? 'ы' : ''}
          </span>
        )}
      </div>
      ...
      <div className="flex items-center gap-2.5 flex-wrap">
        ...
        <div className="chip !text-sm !py-2 text-butter">
          <Icon name="spark" className="w-4 h-4" />
          {coins}
        </div>
        <div className="chip !text-sm !py-2 text-sky">
          <Icon name={WEATHER_ICON[weather.kind] ?? 'sun'} className="w-4 h-4" />
          {weather.label}
        </div>
      </div>
          ...
          return (
            <div key={b.key}>
              <div className="flex items-center justify-between mb-1">
                <span className={`flex items-center gap-1.5 text-[11px] font-extrabold ${low ? 'text-ember' : 'text-cream/65'}`}>
              ...
              <div className="bar-track">
                <div className={`bar-fill ${low ? 'anim-wiggle' : ''}`} style={{ width: `${v}%`, background: b.color }} />
              </div>
            </div>
          );
        })}
      ...
      <div className="flex items-center gap-4">
        <div className="flex-1">
          <div className="flex justify-between text-[11px] font-extrabold text-rose/90 mb-1">
            <span className="flex items-center gap-1.5"><Icon name="heart" className="w-3.5 h-3.5" /> Связь с тобой</span>
            <span>{Math.round(pet.bond)}/100</span>
          </div>
          <div className="bar-track">
            <div className="bar-fill" style={{ width: `${pet.bond}%`, background: 'linear-gradient(90deg,#ff8fb3,#ffaec9)' }} />
          </div>
        </div>
        <div className="w-28">
          <div className="flex justify-between text-[11px] font-extrabold text-mint/90 mb-1">
            <span>Доверие</span><span>{Math.round(pet.trust)}</span>
          </div>
          <div className="bar-track">
            <div className="bar-fill" style={{ width: `${pet.trust}%`, background: 'linear-gradient(90deg,#7fd4ae,#9fe8c9)' }} />
          </div>
        </div>
      </div>
    </div>
