/* Журнал: дневник, сны, память и дерево наследия. */
import { useState } from 'react';
import type { GameState } from '../game/core';
import { RARITY_COLOR } from '../game/core';
      ...
      <div className="flex gap-1 mb-3 bg-night-900/50 rounded-2xl p-1 sm:p-1.5">
        {TABS.map(t => (
          <button key={t.id} onClick={() => setTab(t.id)}
            ...
            <Icon name={t.icon} className="w-4 h-4 sm:w-4.5 sm:h-4.5" /><span className="whitespace-nowrap">{t.label}</span>
          </button>
        ))}
      </div>
        ...
        <div className="space-y-2.5 max-h-[52vh] overflow-y-auto pr-1">
          ...
          {state.dreams.map(d => (
            <div key={d.id} className="card-soft p-3 flex gap-3 items-start">
              <span className="w-9 h-9 rounded-xl flex items-center justify-center shrink-0 text-lilac" style={{ background: 'rgba(200,182,255,0.12)' }}><Icon name="moon" className="w-5 h-5" /></span>
              <div>
        ...
        <div className="space-y-3 max-h-[52vh] overflow-y-auto pr-1">
          <div className="card-soft p-3">
            ...
            <div className="text-[13px] font-bold text-cream/85 space-y-1">
              <p>Имя: <span className="text-butter">{state.owner.name || 'ещё не познакомились'}</span></p>
              <p>Город: <span className="text-sky">{state.owner.city || 'не указан'}</span></p>
          ...
          {state.pet && state.pet.knowledge.length > 0 && (
            <div className="card-soft p-3">
              ...
              <p className="text-[12px] font-bold text-cream/70">Изучено: <span className="text-butter">{state.pet.knowledge.length}</span>. Питомец умнеет с каждой викториной.</p>
            </div>
          )}
          ...
          {[...state.memories].reverse().map(m => (
            <div key={m.id} className="card-soft p-3 flex gap-3 items-start">
              <span className="w-8 h-8 rounded-lg flex items-center justify-center shrink-0" style={{ background: `${KIND_COLOR[m.kind]}1f`, color: KIND_COLOR[m.kind] }}><Icon name={KIND_ICON[m.kind]} className="w-4 h-4" /></span>
        ...
        <div className="space-y-2.5 max-h-[52vh] overflow-y-auto pr-1">
          {state.legacy.length === 0 ? (
            <div className="text-center py-8">
              ...
              <p className="text-sm text-cream/40 font-bold">Дерево наследия пока растёт из настоящего дня.</p>
            </div>
          ) : state.legacy.map(l => (
