/* Полноэкранные состояния: онбординг (яйцо), карточка встречи,
 * «С возвращением», прощание, настройки (без confirm/prompt). */
import { useState } from 'react';
import type { Pet, GameState } from '../game/core';
import { engine } from '../game/engine';
import { speciesOf, abilityOf, RARITY_COLOR } from '../game/core';
...
export function Onboarding() {
  ...
  const startEgg = () => {
    if (name.trim()) engine.state.owner.name = name.trim().slice(0, 20);
    if (city.trim()) engine.state.owner.city = city.trim().slice(0, 30);
    engine.save();
    setPhase('egg');
    sfx.chime();
  };
      ...
      <div className="min-h-dvh flex p-4 overflow-y-auto">
        <div className="card p-5 sm:p-8 max-w-md w-full m-auto text-center anim-fade-up">
          ...
          <p className="text-[13px] font-bold text-cream/60 mt-2 leading-relaxed">
            Где-то между сном и рассветом вас ждёт маленькое существо. Оно будет расти, мечтать, скучать — и помнить вас.
          </p>
          <div className="mt-4 text-left card-soft p-3.5 space-y-1.5">
            ...
            {([
              ['heart', 'Гладьте, купайте, убирайте — на всё есть анимации.'],
              ['spark', 'Кнопки под сценой: кухня, уборка, купание, сон, учёба, прогулка.'],
              ['moon', 'Закройте игру — питомец продолжит жить и видеть сны.'],
              ['diary', 'Вкладки: забота, игры, болталка, дневник.'],
          ...
          <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-2.5 text-left">
            <div>
              ...
              <input className="input-soft mt-1.5" placeholder="Ваше имя" value={name} onChange={e => setName(e.target.value)} onKeyDown={e => e.key === 'Enter' && startEgg()} />
            </div>
            <div>
              <label className="text-[11px] font-black text-cream/50 uppercase tracking-wider">Ваш город?</label>
              <input className="input-soft mt-1.5" placeholder="Например, Казань" value={city} onChange={e => setCity(e.target.value)} onKeyDown={e => e.key === 'Enter' && startEgg()} />
            </div>
            <p className="sm:col-span-2 text-[10.5px] font-bold text-cream/35 leading-snug -mt-0.5">Необязательно — но тогда за окном будет ваша настоящая погода, а на прогулках — улицы вашего города.</p>
          </div>
          <button className="btn btn-primary w-full mt-5" onClick={startEgg}><Icon name="star" className="w-5 h-5" />Найти яйцо</button>
        </div>
      </div>
    );
  }
          ...
          <svg className="w-[min(56vw,200px)] h-auto" viewBox="0 0 200 230">
            ...
            {taps >= 5 && (
              <g style={{ animation: 'popIn 0.4s ease both' }}>
                <circle cx="86" cy="112" r="7" fill="#2b1d33" /><circle cx="116" cy="112" r="7" fill="#2b1d33" />
                <circle cx="88" cy="109" r="2.5" fill="#fff" /><circle cx="118" cy="109" r="2.5" fill="#fff" />
              </g>
            )}
          </svg>
...
/* ================= КАРТОЧКА ВСТРЕЧИ ================= */
export function RevealSheet({ pet, onDone }: { pet: Pet; onDone: () => void }) {
  const [name, setName] = useState(pet.name);
  const sp = speciesOf(pet.dna.species);
  const ability = abilityOf(pet.dna);
  return (
    <div className="fixed inset-0 z-50 flex p-4 overflow-y-auto bg-night-950/85 anim-fade">
      <div className="card max-w-md w-full m-auto anim-fade-up max-h-[92dvh] overflow-x-hidden overflow-y-auto no-scrollbar">
        <div className="p-5 sm:p-6 text-center">
          ...
          <div className="flex justify-center my-2"><div className="anim-pop"><PetSprite pet={pet} size="180px" /></div></div>
          ...
          <div className="mt-3 card-soft p-3 text-left">
            <div className="flex items-center gap-2 text-butter mb-1"><Icon name="spark" className="w-4 h-4" /><span className="text-[12px] font-black uppercase tracking-wider">{ability.name}</span></div>
          ...
          <button className="btn btn-primary w-full mt-4" onClick={() => {
            ...
            sfx.chime();
            onDone();
          }}><Icon name="heart" className="w-5 h-5" />Начать жизнь вместе</button>
        </div>
      </div>
    </div>
  );
...
/* ================= С ВОЗВРАЩЕНИЕМ ================= */
export function WelcomeBack({ awayMs, events, line, petName }: { awayMs: number; events: { icon: string; text: string }[]; line: string; petName: string }) {
  const m = Math.floor(awayMs / 60000);
  const away = m < 60 ? `${m} мин` : m < 60 * 24 ? `${Math.floor(m / 60)} ч ${m % 60} мин` : `${Math.floor(m / 1440)} дн`;
  return (
    <div className="fixed inset-0 z-50 flex p-4 bg-night-950/90 anim-fade overflow-y-auto">
      <div className="card max-w-md w-full m-auto p-4 sm:p-5 anim-pop">
        <div className="text-center mb-3">
          ...
          <h3 className="font-display text-xl font-bold text-butter text-glow mt-1">Вас не было {away}</h3>
        </div>
        ...
        <div className="mt-4 space-y-2">
          ...
          {events.length === 0 && <p className="text-[12.5px] font-bold text-cream/55 card-soft p-3">{petName} тихо дремал и видел маленький тёплый сон.</p>}
          {events.map((e, i) => (
            <div key={i} className="card-soft p-3 flex items-start gap-3" style={{ animation: `toastIn 0.4s ease both ${i * 0.12}s` }}>
              <span className="w-8 h-8 rounded-lg flex items-center justify-center text-butter shrink-0" style={{ background: 'rgba(255,217,142,0.1)' }}><Icon name={e.icon} className="w-4 h-4" /></span>
              <p className="text-[12.5px] font-bold text-cream/75 leading-snug">{petName} {e.text}.</p>
            </div>
          ))}
        </div>
        <button className="btn btn-primary w-full mt-4" onClick={() => { engine.hugOnReturn(); sfx.purr(); }}><Icon name="heart" className="w-5 h-5" />Обнять</button>
      </div>
    </div>
  );
}
...
/* ================= ПРОЩАНИЕ ================= */
export function Farewell({ entry, onNewGen, onKeep }: { entry: NonNullable<GameState['pendingFarewell']>; onNewGen: () => void; onKeep: () => void }) {
  return (
    <div className="fixed inset-0 z-50 flex p-4 anim-fade overflow-y-auto" style={{ background: 'radial-gradient(ellipse at 50% 30%, #2a2547 0%, #0c1220 75%)' }}>
      <div className="card max-w-md w-full m-auto p-5 sm:p-6 text-center anim-pop">
        <div className="mx-auto mb-3 w-20 h-20 rounded-full flex items-center justify-center anim-float" style={{ background: `${entry.colorPrimary}26`, boxShadow: `0 0 60px ${entry.colorPrimary}59` }}>
        ...
        <button className="btn btn-primary w-full mt-4" onClick={onNewGen}><Icon name="spark" className="w-5 h-5" />Найти новое яйцо</button>
...
export function SettingsModal({ state, onClose }: { state: GameState; onClose: () => void }) {
  ...
  const doExport = () => {
    setSaveCode(engine.exportSave());
    if (navigator.clipboard?.writeText) navigator.clipboard.writeText(engine.exportSave()).then(() => flash('Сейв скопирован!')).catch(() => flash('Код ниже — скопируйте вручную', false));
    else flash('Код ниже — скопируйте вручную', false);
  };
  const doImport = () => {
    if (!importCode.trim()) { flash('Вставьте код сейва', false); return; }
    if (engine.importSave(importCode)) { flash('Сейв загружен!'); setShowImport(false); setImportCode(''); }
    else flash('Не удалось прочитать сейв', false);
  };
  const doExportBrain = () => {
    const code = engine.exportBrain();
    if (!code) { flash('Мозг ещё не вырос', false); return; }
    if (navigator.clipboard?.writeText) navigator.clipboard.writeText(code).then(() => flash('Модель мозга скопирована!')).catch(() => prompt('Скопируйте модель:', code));
    else prompt('Скопируйте модель:', code);
  };
  const doImportBrain = () => {
    if (!brainCode.trim()) { flash('Вставьте код модели', false); return; }
    if (engine.importBrain(brainCode)) { flash('Мозг загружен!'); setShowBrainImport(false); setBrainCode(''); }
    else flash('Не удалось прочитать модель', false);
  };
  const doReset = () => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex p-4 bg-night-950/85 anim-fade overflow-y-auto">
      <div className="card max-w-md w-full m-auto p-4 sm:p-5 anim-pop space-y-3.5 max-h-[92dvh] overflow-y-auto no-scrollbar">
        ...
        {msg && <div className={`card-soft px-3 py-2 text-[12.5px] font-bold ${msg.ok ? 'text-mint' : 'text-peach'}`} style={{ animation: 'toastIn 0.3s ease both' }}>{msg.text}</div>}

        <div className="card-soft p-3.5 flex items-center justify-between gap-3">
          <div className="flex items-center gap-2.5 text-cream/85"><Icon name={state.settings.sound ? 'soundOn' : 'soundOff'} className="w-5 h-5 text-sky" /><span className="text-[13px] font-extrabold">Звук</span></div>
          <button className={`btn !py-1.5 !px-4 !text-xs ${state.settings.sound ? 'btn-mint' : 'btn-ghost'}`} onClick={() => engine.setSound(!state.settings.sound)}>{state.settings.sound ? 'Вкл' : 'Выкл'}</button>
        </div>

        <div className="card-soft p-3.5">
          <div className="flex items-center gap-2.5 text-cream/85 mb-2"><Icon name="compass" className="w-5 h-5 text-sky" /><span className="text-[13px] font-extrabold">Город (для погоды и прогулок)</span></div>
          <div className="flex gap-2">
            ...
            <button className="btn btn-sky !py-2 !px-3.5 !text-xs" onClick={() => { engine.setCity(city); engine.refreshWeather(); flash(city.trim() ? `Погода для: ${city.trim()}` : 'Город убран'); }}><Icon name="check" className="w-4 h-4" /></button>
          </div>
        </div>

        <div className="card-soft p-3.5">
          <div className="flex items-center justify-between gap-2 mb-1.5">
            <div className="flex items-center gap-2.5 text-cream/85"><Icon name="brain" className="w-5 h-5 text-lilac" /><span className="text-[13px] font-extrabold">Нейросеть питомца</span></div>
            <span className={`chip !text-[10px] ${brain.ready ? 'text-mint' : 'text-cream/40'}`}>{brain.ready ? `знает ${brain.words} слов` : 'растёт…'}</span>
          </div>
          <p className="text-[10.5px] font-bold text-cream/40 leading-snug mb-2.5">Учится на разговорах. Обработано {brain.tokens.toLocaleString('ru-RU')} слов. Выгружается отдельной моделью.</p>
          ...
          {showBrainImport && (
            <div className="anim-fade-up space-y-2 mt-2">
              <textarea value={brainCode} onChange={e => setBrainCode(e.target.value)} placeholder="Вставьте код модели мозга…" className="input-soft !text-[10px] !leading-relaxed h-20 resize-none no-scrollbar" />
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          <button className="btn btn-lilac !py-2.5 !text-xs" onClick={() => setShowImport(v => !v)}><Icon name="import" className="w-4 h-4" />Импорт</button>
        </div>
        {saveCode && <textarea readOnly value={saveCode} onFocus={e => e.currentTarget.select()} className="input-soft !text-[10px] h-20 resize-none no-scrollbar" />}
        {showImport && (
          <div className="anim-fade-up space-y-2">
            <textarea value={importCode} onChange={e => setImportCode(e.target.value)} placeholder="Вставьте код сейва…" className="input-soft !text-[10px] h-20 resize-none no-scrollbar" />
        ...
        <button className={`btn w-full !text-xs transition-all ${confirmReset ? 'btn-danger' : 'btn-ghost'}`} onClick={doReset}>
