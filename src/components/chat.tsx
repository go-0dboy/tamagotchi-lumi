/* Болталка: чат с памятью и нейросетью, индикатор «печатает…»,
 * быстрые подсказки, дыхание и фокус-таймер. */
import { useEffect, useRef, useState } from 'react';
import type { GameState } from '../game/core';
import { engine } from '../game/engine';
import { AFFIRMATIONS } from '../game/content-data';
import { sfx } from '../game/core';
import Icon from './icons';

const QUICK = ['Привет!', 'Как дела?', 'Что делал сегодня?', 'Кто ты?', 'Расскажи факт'];
...
export default function ChatPanel({ state }: { state: GameState }) {
  ...
  /* индикатор печати: включаем при отправке, выключаем при ответе */
  const lastOwnerAt = useRef(0);
  useEffect(() => {
    const last = state.chat[state.chat.length - 1];
    if (last?.from === 'owner') { lastOwnerAt.current = last.at; setTyping(true); }
    else if (last?.from === 'pet' && last.at > lastOwnerAt.current) setTyping(false);
  }, [state.chat]);
  useEffect(() => {
    if (!typing) return;
  ...
  useEffect(() => {
    ...
    const loopFn = () => {
      timer = setTimeout(() => {
        ...
        if (oi >= order.length) {
          oi = 0; c++; setCycles(c);
          if (c >= 4) { setBreathing(false); engine.setBubble('Вот это спокойствие!'); engine.save(); return; }
        }
        setBreathPhase(order[oi].ph); sfx.bubble(); loopFn();
      }, order[oi].ms);
    };
    loopFn();
  ...
  return (
    <div className="card p-3.5 sm:p-4 anim-fade-up">
      ...
      <div className="flex gap-1 mb-3 bg-night-900/50 rounded-2xl p-1 sm:p-1.5">
        <button onClick={() => setMode('chat')} className={`flex-1 flex items-center justify-center gap-1.5 py-2 rounded-xl text-[11px] font-extrabold transition-all ${mode === 'chat' ? 'bg-night-700 text-butter' : 'text-cream/50'}`}><Icon name="chat" className="w-4 h-4" />Болталка</button>
        <button onClick={() => setMode('calm')} className={`flex-1 flex items-center justify-center gap-1.5 py-2 rounded-xl text-[11px] font-extrabold transition-all ${mode === 'calm' ? 'bg-night-700 text-mint' : 'text-cream/50'}`}><Icon name="moon" className="w-4 h-4" />Тишина</button>
      </div>

      {mode === 'chat' ? (
        <>
          <div ref={logRef} className="h-[34vh] overflow-y-auto no-scrollbar space-y-2 pr-1 mb-3">
            {state.chat.length === 0 && (
          ...
          <div className="flex gap-1.5 flex-wrap mb-2">
            {QUICK.map(qq => (
              <button key={qq} onClick={() => send(qq)} className="chip !text-[10.5px] !py-1.5 hover:border-butter/50 hover:text-butter transition-all active:scale-95">{qq}</button>
            ))}
          </div>
        ...
        <div className="space-y-3">
          <button onClick={() => { setBreathing(true); sfx.pop(); }}
            ...
            <span className="w-11 h-11 rounded-2xl flex items-center justify-center text-mint shrink-0 group-hover:scale-110 transition-transform" style={{ background: 'rgba(159,232,201,0.1)' }}><Icon name="moon" className="w-5.5 h-5.5" /></span>
            <span>
          ...
          <div className="card-soft p-4">
            <div className="font-display font-bold text-[13.5px] mb-1">Фокус-таймер</div>
            <p className="text-[10.5px] font-bold text-cream/45 mb-2.5">Учимся или работаем вместе. {pet.name} посидит рядом.</p>
            {focusLeft ? (
              <div className="text-center">
                <div className="font-display font-bold text-3xl text-sky tabular-nums">{focusLeft}</div>
                <p className="text-[10.5px] font-bold text-cream/40 mt-1">{pet.name} тихо сидит рядом…</p>
              </div>
            ) : (
              <div className="flex gap-2">
                {[5, 15, 25].map(m => (
                  <button key={m} onClick={() => { engine.startFocus?.(m); sfx.pop(); }} className="flex-1 btn btn-sky !py-2 !text-xs">{m} мин</button>
                ))}
              </div>
            )}
          </div>
          ...
          <div className="card-soft p-4">
            <div className="font-display font-bold text-[13.5px] mb-1">Аффирмация</div>
            <p className="text-[12px] font-bold text-cream/70 leading-relaxed italic">«{affirmation}»</p>
            <button className="btn btn-lilac !py-2 !px-3 !text-xs mt-2.5" onClick={() => { setAffirmation(AFFIRMATIONS[Math.floor(Math.random() * AFFIRMATIONS.length)]); sfx.bubble(); }}>Ещё одну</button>
          </div>
        </div>
      )}
    </div>
