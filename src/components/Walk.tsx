    ...
    <div className="fixed inset-0 z-50 flex p-4 bg-night-950/90 anim-fade overflow-y-auto" onClick={step === 'result' ? onClose : undefined}>
      {step === 'pick' && (
        <div className="card max-w-lg w-full m-auto p-4 sm:p-5 anim-pop space-y-3" onClick={e => e.stopPropagation()}>
          ...
          <p className="text-[12px] font-bold text-cream/50 -mt-1">У каждого места — своя дорога и своя история. Нужна энергия (12+).</p>
          ...
          {/* «Улицы города» — растянутая карточка, как «Случайный факт» в учёбе */}
          {cityLoc && (
            <button onClick={() => go(cityLoc)}
              className="w-full card-soft p-3.5 text-left flex items-center gap-3 hover:border-peach/50 active:scale-[0.98] transition-all group">
              ...
              <Icon name="walk" className="w-4 h-4 text-cream/30 group-hover:text-peach transition-colors" />
            </button>
          )}
          {!cityLoc && (
            <p className="text-[10.5px] font-bold text-cream/35 leading-snug">Укажите город в Настройках — и здесь появится прогулка по его улицам.</p>
          )}
        </div>
      )}
  ...
  return (
    <div className="relative w-full max-w-2xl m-auto rounded-[28px] overflow-hidden border border-sky/15 anim-fade"
      ...
      <div className="absolute inset-x-0 bottom-0 h-[15%]" style={{ background: 'linear-gradient(180deg,#3a3050,#2b2140)' }}>
        <div className="absolute inset-x-0 top-1/2 h-1 anim-walk-strip" style={{
          width: '200%', animationDuration: '2.5s',
